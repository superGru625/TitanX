...
import { useStakingAddress, useStakingTokenAddress } from "../../hooks";
...
const Rewards = ({ refreshStakeInfo }) => {
    ...
    const loadStakeData = async (accountAd) => {
        try {
            ...
            const _stakeStorage = await _stakingContract.methods
                .StakerStorage(account)
                .call();
            const _bnbReward = web3.utils.fromWei(
                _stakeStorage.tokensEarned.toString()
            );
    ...
    const reInvestAll = async () => {
        try {
            await stakingContract.methods.reInvestAll().send({
                from: account,
            });
            refreshStakeInfo();
            showToast("SUCCESS", "Successfully COMPOUNDED your token!");
        } catch (e) {
    ...
    const unstakeToken = async () => {
        try {
            const _amount = numberofTokens * 10 ** decimals;
    ...
    const withdraw = async () => {
        try {
            await stakingContract.methods.withdrawRewards().send({
                from: account,
            });
        } catch (err) {
            showToast("ERROR", "Something went wrong!");
