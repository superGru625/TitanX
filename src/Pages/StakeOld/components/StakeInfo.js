...
import ABI_STAKING from "../../../abis/staking/ABI_Staking_Old.json";
    ...
    const loadStakeData = async () => {
        try {
            ...
            await new Promise(function (resolve, reject) {
                contract.methods
                    .stakeValue(account)
                    .call({}, async (err, _stakeValue) => {
                        if (err) reject(err);
                        const tokensStaked = _stakeValue / 10 ** _decimals;
                ...
                contract.methods.totalStakers().call({}, (err, tStaker) => {
                    if (err) reject(err);
